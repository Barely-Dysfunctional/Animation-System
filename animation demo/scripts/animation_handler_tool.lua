...
function init()
    ...
    key = "level.shapeanimhandlers."..name
    while HasKey(key) do
        key = key.."(2)"
    end
...
function tick(dt)
    ...
    if GetString("game.player.tool") == name then
        origin = GetToolBody()
        rig = GetBodyShapes(origin)

        --reverse rig because tools are stupid
        rev = {}
        for i=#rig, 1, -1 do
	        rev[#rev+1] = rig[i]
        end
        ...
        --for i=1, #rig do
        --    DebugCross(GetShapeWorldTransform(rig[i]).pos)
        --    DebugPrint(i..", "..rig[i])
        --end
    ...
    --if currentKeyframe < #values then
        ...
        if timer > (1/framerate) then
            ...
            for id, objectValues in ipairs(framevalues[2]) do
                ...
                startTrans = currentTrans
                --DebugPrint("startTrans: "..TransformStr(startTrans))
                --endTrans = TransformToParentTransform(originTrans, nextTrans) 
                endTrans = nextTrans
                --DebugPrint("endTrans: "..TransformStr(endTrans))
                ...
                SetShapeLocalTransform(rig[id], Transform(newPos, newRot))
